            ...
            config.wayland.windowManager.hyprland = mkIf config.gui.hyprland.enable {
              ...
              extraConfig =
                let
                  ...
                  monitorString =
                    with osConfig.machine.monitor;
                    "${name},${toString width}x${toString height}@${toString refreshRate},0x0,${toString scale}";
                  ...
                  vrr = toString (if osConfig.machine.monitor.variableRefreshRate then 1 else 0);

                  activeBorder = if osCfg.light-theme then "0xfffbf1c7" else "0xff3c3836";

                  inactiveBorder = if osCfg.light-theme then "0xffebdbb2" else "0xff282828";

                  groupBorder = if osCfg.light-theme then "0xffd5c4a1" else "0xff504945";

                  groupBorderActive = if osCfg.light-theme then "0xffbdae93" else "0xff665c54";

                  background =
                    if osCfg.light-theme then
                ...
                ''
                  ...
                  input {
                    kb_layout=${osConfig.machine.keyboard.layout}
                    kb_variant=${osConfig.machine.keyboard.variant}
                    repeat_rate=50
                    repeat_delay=200
                  ...
                  decoration {
                    rounding=${toString (4 * osConfig.machine.monitor.scale)}
                    drop_shadow=false
                    dim_inactive=true
                    dim_strength=0.2
                    blur {
                  ...
                  # Screenshot
                  bind=SUPER,g,exec,${grim} - | ${swappy} -f -
                  bind=SUPERSHIFT,g,exec,${grim} -g "$(${slurp} -b 28282877 -c d64d0e)" - | ${swappy} -f -
