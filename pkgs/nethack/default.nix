...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    sed \
      -e 's,^WINQT4LIB =.*,WINQT4LIB = `pkg-config Qt5Gui --libs` `pkg-config Qt5Widgets --libs` `pkg-config Qt5Multimedia --libs`,' \
      -e '/rm -f $(MAKEDEFS)/d' \
    ...
    sed \
      ...
      -e 's,NHCFLAGS+=-DCOMPRESS[^ ]*,NHCFLAGS+=-DCOMPRESS=\\"${gzip}/bin/gzip\\" -DCOMPRESS_EXTENSION=\\".gz\\",' \
    ...
    sed \
      ...
      -e '/^endif   # QTDIR/a QTDIR=${qt5.qtbase.dev}' \
      -e 's,PKG_CONFIG_PATH=$(QTDIR)/lib/pkgconfig,,' \
      -e 's,NHCFLAGS+=-DCOMPRESS[^ ]*,NHCFLAGS+=-DCOMPRESS=\\"${gzip}/bin/gzip\\" -DCOMPRESS_EXTENSION=\\".gz\\",' \
